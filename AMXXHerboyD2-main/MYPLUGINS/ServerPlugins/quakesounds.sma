...
public QuakeSoundsPrint(Player)
{
	...
	if (g_bConnected[Player])
		client_print(Player, print_chat, ">> Ird be 'sounds' , hogy ki/be kapcsold a Quake hangokat.");
}
...
qs_client_cmd(Target, Rules[], any:...)
{
	...
	// SPECIFIED CLIENT
	//
	if (Target > 0 && g_bConnected[Target] && !g_bBOT[Target] && !g_bHLTV[Target] && !sm_get_quakesounds(Target))
