...
public actionSlapMenu(id, key)
{
	switch (key)
	{
		...
		default:
		{
			...
			{
				sk_chat(0, "^4%s: ^3%s^1 megütötte ^4%s^1-t ^3%i^1 sebzéssel.", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", plmenu_name[id], plmenu_name[player], g_menuSettings[id])
				sk_log("AdminActivity", fmt("[AMX_SLAPMENU] %s %s slapped %s with %i damage", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", plmenu_name[id], plmenu_name[player], g_menuSettings[id]))
			...
			{
				sk_chat(0, "^4%s: ^3%s^1 megölte ^4%s^1-t", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", plmenu_name[id], plmenu_name[player])
				sk_log("AdminActivity", fmt("[AMX_SLAPMENU] %s %s slayed %s", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", plmenu_name[id], plmenu_name[player]))
...
public actionKickMenu(id, key)
{
	switch (key)
	{
		...
		default:
		{
			...
			new userid2 = get_user_userid(player);
			//sk_chat(0, "^4%s: ^3%s^1 kirúgta a szerverről ^4%s-t. Indok: ^4Kicked By AdminMenu", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", plmenu_name[id], plmenu_name[player])
			sk_log("AdminActivity", fmt("[AMX_KICKMENU] %s %s kick %s ", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", plmenu_name[id], plmenu_name[player]))

			//skbs_kick_user(player, id, "KickedByMenu")
...
{
	get_user_name(id, plmenu_name[id], charsmax(plmenu_name))
...
public actionTeamMenu(id, key)
{
	switch (key)
	{
		...
		default:
		{
			...
			sk_chat(0, "^4%s: ^3%s^1 áthelyezte ^4%s^1-t ide: ^4%s", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", plmenu_name[id], plmenu_name[player], g_CSTeamNames[destTeamSlot])
			sk_log("AdminActivity", fmt("[AMX_TEAMMENU]  %s %s transfered %s to %s", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", plmenu_name[id], plmenu_name[player], g_CSTeamNames[destTeamSlot]))
