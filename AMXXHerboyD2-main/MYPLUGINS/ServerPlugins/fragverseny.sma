...
#define PLUGIN_NAME		"Fragverseny"
#define PLUGIN_VERSION	"1.0"
#define PLUGIN_AUTHOR	"EXILLE"
#define MAX_PLAYERS		32
#define prefixmenu "\r[\wHerBoy\r] \r » \r[ \wFragverseny menü \r]"
#define prefix "^4[^1~^3|^4HerBoy^3|^1~^4] ^3»"
...
public plugin_precache()
{
	...
	precache_generic("sound/hb_fragverseny/1.mp3");
}
...
public plugin_init()
{
	...
	register_clcmd("fragmenu", "openMain")
...
public plugin_natives()
{
  ...
  register_native("hb_fragversenymenu","native_hb_fragversenymenu",1)
  register_native("hb_fragversenymenusettings","native_hb_fragversenymenusettings",1)
}
public native_hb_fragversenymenu(id)
{
    return openMain(id);
}
public native_hb_fragversenymenusettings(id)
{
    return openSettings(id);
}
...
public concmd_RockFinal()
{
	...
	client_print_color(0, print_team_default, "^4[INFO]^1 A ^3FRAGVERSENY INTRO^1 elkezdődik!");
...
public getPlayerSound()
{
	...
	if(is_user_connected(id))
	{
		new volume[33], Float:volumefloat[99]
		get_user_info(id, "MP3Volume", volumefloat, 99);
		copy(playervolumes, 99, volumefloat)
	}
}
...
public fn_PlayMusic()
{
	...
	switch(g_count_)
	{
		case 6:
		{
			...
			set_task(73.5, "SetMP3S", _,_,_, "a", 80)
			client_cmd(0, "mp3 play ^"sound/hb_fragverseny/1.mp3^"");
...
public fn_Messages()
{
	...
	new szMessages[][] = { "Love you just the way you are", "---~ [~|HerBoy|~] ~--", "Üdvözlünk a HerBoy szerveren!", "Jó játékot kívánunk!", "Sok sikert a fragversenyhez!" }
...
public openMain(id)
{
	...
	format(szMenu, charsmax(szMenu), "\r%s ^n\dIdő: %s", prefixmenu, Time)
	...
	//	menu_additem(menu, "Fragverseny beállitások", "3",0)
	}
...
public openSettings(id) {
	new szMenu[121]
	format(szMenu, charsmax(szMenu), "\r%s", prefixmenu)
...
{   
    new szMenu[121], players[32], szTemp[10], pnum, Name[32], sortedPlayers[32]
    get_players(players, pnum)

    for (new i = 0; i < pnum; i++)
        sortedPlayers[i] = players[i];

    for (new i = 0; i < pnum - 1; i++)
    {
        for (new j = i + 1; j < pnum; j++)
        {
            if (kill[sortedPlayers[j]] > kill[sortedPlayers[i]])
            {
                new temp = sortedPlayers[i];
    ...
    format(szMenu, charsmax(szMenu), "\r%s \wJátékosok Ölései", prefixmenu)
    new menu = menu_create(szMenu, "viewer_handler")

    for (new i = 0; i < pnum; i++)
    {
        get_user_name(sortedPlayers[i], Name, charsmax(Name))
        formatex(szMenu, charsmax(szMenu),"%s \d[\yÖLÉS:\r %i\d]", Name, kill[sortedPlayers[i]])
        num_to_str(sortedPlayers[i], szTemp, charsmax(szTemp))
...
public openReseter(id) {
	...
	format(cim, charsmax(cim), "\r%s \yÖlés nullázás", prefixmenu)
