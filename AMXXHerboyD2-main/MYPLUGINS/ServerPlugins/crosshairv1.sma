...
new const MenuKomutlari[][] = { "say /ch", "say_team /celkereszt","say ch", "say_team ch","say celkereszt", "say_team celkereszt" };
...
public plugin_init(){
	register_plugin("CS:GO Crosshair", "2.4", "csfweed")
	...
	register_message(g_Messages[g_iMsg_SetFOV], "message_setfov") 
		...
		new cvarayar = create_cvar("alapertelmezett_kurzor","1", FCVAR_NONE, "A kurzort a játékosok számára kell kiválasztani (0-11)")
		bind_pcvar_num(cvarayar, giristecrosshair)
	#else
		giristecrosshair = get_pcvar_num(register_cvar("alapertelmezett_kurzor", "1"))
	#endif
...
public csgobilgi()
{
	client_print_color(0,0, "^4[CS:GO] ^1Szeretnél CS:GO-s célkeresztet? Írd a be chatre ^3/celkereszt ^1vagy ^3ch^1.")
}
...
public genelmenu(id)
{
	...
	new menu = menu_create("\r[CS:GO] \wCélkereszt \wBeállítás","genelmenu2")
	new ekleme[128]
	if(ImlecKontrol[id][SeciliCrosshair] == -1)
		formatex(ekleme,127,"\wCélkereszt \d[\rAlap\d]")
	else
		formatex(ekleme,127,"\wCélkereszt \d[\r%s\d]",genel_bilgiler[ImlecKontrol[id][SeciliCrosshair]][0][0])
 
	menu_additem(menu,ekleme,"1",0)
	formatex(ekleme,127,"\wSniper Scope \d[\r%s\d]",ImlecKontrol[id][SniperKontrol] ? "BE":"\dKI")
	menu_additem(menu,ekleme,"2",0)
	formatex(ekleme,127,"\wAUG & SG552 Scope \d[\r%s\d]",ImlecKontrol[id][AssaultKontrol] ? "BE":"\dKI")
	menu_additem(menu,ekleme,"3",0)
	formatex(ekleme,127,"\wFOV \d[\r%i \w/ \r%i\d]^n^n",ImlecKontrol[id][FovKontrol],MAX_FOV)
...
public imlecsecim(id)
{
	...
	new cevir[10];
	new menu = menu_create("\r[CS:GO] \wCélkereszt \wBeállítás^n","imlecsecim2")
	num_to_str(MAX_CROSSHAIR+1,cevir,9)
	menu_additem(menu,"\wNormál Célkereszt",cevir,0)
...
public imlecsecim2(id,menu,item)
{
	...
	if(key > MAX_CROSSHAIR)
	{
		...
		HookCurWeapon2(id)
		client_print_color(id,id,"^4[CS:GO] ^1Célkereszted visszaállítottad az alapra!")
	}
	else
	{
