...
public plugin_init() {
	...
	register_clcmd("say /ch", "@clcmd_crosshair");
	...
	register_dictionary("crosshairv2.txt");
}
...
@clcmd_crosshair(const id) {
	new menu = menu_create(fmt("\w[\rHerBoy\w] \y» \w%L", id, "CH_MENU_CROSSHAIR"), "@clcmd_crosshair_handler");

	if(g_crosshair_type[id]) {
		menu_additem(menu, fmt("%L %s", id, "CH_MENU_TYPE", crosshairs[g_crosshair_type[id] - 1][0]), "1");
		menu_additem(menu, fmt("%L %L", id, "CH_MENU_COLOR", id, colors[g_crosshair_color[id]]), "2");
		menu_additem(menu, fmt("%L", id, "CH_MENU_RESET"), "3");
	}
	else {
		menu_additem(menu, fmt("%L", id, "CH_MENU_TYPE_DEFAULT"), "1");
	}
