...
{
	get_user_name(id, mapm_names[id], charsmax(mapm_names))
}
public autoRefuse()
{
...
public checkVotes(id)
{
	...
	if (iResult >= iRatio)
	{
		...
		sk_chat(0, "A szavazás sikeres, pályaváltás a következőre: ^4%s", tempMap)
	}
	...
	if (g_choosed != -1)
	{
		...
		sk_chat(0, "A szavazás sikertelen.")
...
public actionVoteMapMenu(id, key)
{
	...
	switch (key)
	{
		case 7:
		{
			...
			sk_chat(0, "^4%s: ^3%s^1 pályaszavazás létrehozva.", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", mapm_names[id])
			new tempMapA[32];
			new tempMapB[32];
			new tempMapC[32];
			new tempMapD[32];
...
public actionMapsMenu(id, key)
{
	switch (key)
	{
		...
		default:
		{
			...
			sk_chat(0, "^4%s: ^3%s^1 pályaváltás a következőre: ^4%s", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", mapm_names[id], tempMap)
			sk_log("AdminActivity", fmt("[AMX_MAPMENU] %s %s change map to %s ", id > 0 ? Admin_Permissions[get_user_adminlvl(id)][0] : "Server RCON", mapm_names[id], tempMap))
			set_task(2.0, "delayedChange", 0, tempMap, strlen(tempMap) + 1)
