...
{
	precache_model("models/multimod_parachute.mdl")
}
...
public HandleSay(id)
{
	...
	if (containi(args, "parachute") != -1) {
		if (gCStrike) client_print(id, print_chat, "[AMXX] Ejtoernyo parancsok: buy_parachute, sell_parachute, give_parachute")
		client_print(id, print_chat, "[AMXX] Ejtoernyo hasznalata E-vel!")
	}
...
public buy_parachute(id)
{
	...
	if (!get_pcvar_num(pEnabled)) {
		client_print(id, print_chat, "[AMXX] Ejtoernyo kikapcsolva")
		return PLUGIN_HANDLED
	}

	if (has_parachute[id]) {
		client_print(id, print_chat, "[AMXX] Neked mar van ejtoernyod")
		return PLUGIN_HANDLED
	}
	...
	if (money < cost) {
		client_print(id, print_chat, "[AMXX] Keves a penzed hogy ejtoenryot vehess - Ara $%i", cost)
		return PLUGIN_HANDLED
	}
...
public sell_parachute(id)
{
	...
	if (!has_parachute[id]) {
		client_print(id, print_chat, "[AMXX] Neked nincs ejtoernyod, hogy eladd")
		return PLUGIN_HANDLED
	}

	if (access(id,PARACHUTE_LEVEL)) {
		client_print(id, print_chat, "[AMXX] Nem tudod eladni az ejtoernyod")
		return PLUGIN_HANDLED
	}
	...
	client_print(id, print_chat, "[AMX] Eladtad az ejtoernyod $%d", sellamt)
...
public give_parachute(id,args[])
{
	...
	if (!has_parachute[id]) {
		client_print(id, print_chat, "[AMXX] Nincs ejtoernyod")
		return PLUGIN_HANDLED
	}
	...
	if(has_parachute[player]) {
		client_print(id, print_chat, "[AMXX] %s Mar van ejtoernyod.", pl_name)
		return PLUGIN_HANDLED
	}
	...
	client_print(id, print_chat, "[AMXX] Eladtad az ejtoernyod %s.", pl_name)
	client_print(player, print_chat, "[AMXX] %s ejtoernyot adott neked.", id_name)
...
public admin_give_parachute(id, level, cid) {
	...
	if (arg[0]=='@'){
		...
		if (inum == 0) {
			console_print(id,"Nincs ilyen ember")
			return PLUGIN_HANDLED
		}
		...
		switch(get_cvar_num("amx_show_activity"))	{
			case 2:	client_print(0,print_chat,"ADMIN %s: adott ejtoernyot ^"%s^" players",name,arg[1])
			case 1:	client_print(0,print_chat,"ADMIN: adott ejtoernyot ^"%s^" players",arg[1])
		}

		console_print(id,"[AMXX] Te adtal ejtoernyot ^"%s^" players",arg[1])
		log_amx("^"%s<%d><%s><>^" adott ejtoernyot ^"%s^"", name,get_user_userid(id),authid,arg[1])
	}
	else {
		...
		switch(get_cvar_num("amx_show_activity")) {
			case 2:	client_print(0,print_chat,"ADMIN %s: adott ejtoernyot ^"%s^"",name,name2)
			case 1:	client_print(0,print_chat,"ADMIN: adott ejtoernyot ^"%s^"",name2)
		}

		console_print(id,"[AMXX] Te adtal ejtoernyot ^"%s^"", name2)
		log_amx("^"%s<%d><%s><>^" adott ejtoernyot ^"%s<%d><%s><>^"", name,get_user_userid(id),authid,name2,get_user_userid(player),authid2)
	}
	return PLUGIN_HANDLED
}
	...
	if (button & IN_USE) {
		...
		if (velocity[2] < 0.0) {
			...
			if(para_ent[id] <= 0) {
				...
				if(para_ent[id] > 0) {
					entity_set_string(para_ent[id],EV_SZ_classname,"multimod_parachute")
					entity_set_edict(para_ent[id], EV_ENT_aiment, id)
					entity_set_edict(para_ent[id], EV_ENT_owner, id)
					entity_set_int(para_ent[id], EV_INT_movetype, MOVETYPE_FOLLOW)
					entity_set_model(para_ent[id], "models/multimod_parachute.mdl")
