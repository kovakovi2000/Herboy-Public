...
*/
//https://github.com/EXILLE1312/HerboyD2/blob/dbbb090aafad75ae228671c3aa7ac2a644e19062/Server%20Plugins/vanish.sma
#include <amxmodx>
#include <amxmisc>
#include <reapi>
...
new g_sPlayerName[MAX_CLIENTS + 1][32];
...
public plugin_init()
{
	...
	register_clcmd("vanish", "ClCmd_Spectate", ADMIN_ADMIN, "Makes <id> invisible.");
#endif
...
public SV_WriteFullClientUpdate(const id, buffer, const receiver)
{
	...
	{
		if(~get_user_flags(receiver) & ADMIN_BAN)
			set_key_value(buffer, "", "");
		else
			set_key_value(buffer, "name", g_sPlayerName[id]);
...
public client_putinserver(id)
{
	...
	format(g_sPlayerName[id], charsmax(g_sPlayerName[][]), "[!]%s", g_sPlayerName[id]);
