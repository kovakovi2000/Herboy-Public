document.addEventListener("DOMContentLoaded", function () {
  document.body.addEventListener("click", function (event) {
    ...
    // Check for change ban button clicks
    if (event.target.classList.contains("change-ban-btn")) {
      event.stopPropagation();
      const banId = event.target.getAttribute("data-ban-id");
      closeExistingModal(); // Close any existing modal before opening a new one
      handleChangeBanButtonClick(event.target);
    }

    // Check for about ban button clicks
...
function handleChangeBanButtonClick(button) {
  ...
  const adminNameCard = button.getAttribute("data-admin-name-card"); // Changed to get NameCard HTML
