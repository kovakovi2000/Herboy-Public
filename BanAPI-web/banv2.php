...
$runtime = array();
$final_url = "https://herboyd2.hu/ban_api/motd_herboy.html?ver5634345325";
$additional = "";
...
define('DISCORD_WEBHOOK_URL', 'https://discord.com/api/webhooks/1303786825911111680/7LB8u2doZK0stQR4VjFJEONE5UPPbJp-iiUeIm0MEVn2THmh305ES2e5zzYX0JC-B5Pz'); // Replace with your webhook URL
...
define('REQ_HASH_SALT', "AmxxBanSystemUUID__0191a782-3b7f-7107-806a-fbf98a2c65f8");
...
)
{
    ...
    triggerCustomError(E_USER_WARNING,"UNSET URI PARAMETER\n".$_SERVER["REQUEST_URI"], __FILE__, __LINE__);
    return;
}

$g_steam = filter_input(INPUT_GET, 'st', FILTER_SANITIZE_FULL_SPECIAL_CHARS); $g_steam = empty($g_steam) ? $g_steam : strtoupper($g_steam);
$g_ipv4 = filter_input(INPUT_GET, 'i', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
$g_validator = filter_input(INPUT_GET, 'va', FILTER_VALIDATE_INT);
$g_server = filter_input(INPUT_GET, 'se', FILTER_VALIDATE_INT);
$gid = filter_input(INPUT_GET, 'gi', FILTER_VALIDATE_INT);
$g_version = filter_input(INPUT_GET, 've', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
$local = REQ_HASH_SALT.";{$g_steam};{$g_ipv4};{$g_validator};{$gid};{$g_version}";
$g_new = false;

if(md5($local) != $_GET['h'])
{
    triggerCustomError(E_USER_WARNING,"FAILED HASHCHECK URI\n".$_SERVER["REQUEST_URI"]."\nLocal: ".str_replace(REQ_HASH_SALT, "", $local), __FILE__, __LINE__);
    $player_cluster = array();
    return;
}
...
function get_via_proxy($url) {
    global $proxies, $proxyfails_total;

    if($proxyfails_total >= 10)
        return null;
...
function get_ip_data($checkip) {
    ...
    try {
        $stmt = $pdoban->prepare("SELECT * FROM ip_data WHERE ip = :ip LIMIT 1");
        ...
        if ($result) {
            ...
            if ($interval < IP_API_RECHECK_DAYS) {
                ...
                {
                    try {
                        running_time("get_ip_data", "get_via_normal");
                        $ch = curl_init($url);
                        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
function add_ip(&$player_cluster, $ip, $last = PHP_INT_MAX) {
    ...
    // Check if the $last is a valid date and get the interval in days
    if (is_string($last) && strtotime($last) !== false) {
        ...
        if ($interval < MAX_IP_AGE_DAYS) {
            return;
        }
    }

    // Get IP data (from external source or internal logic)
    $ip_data = get_ip_data($ip);

    // If no IP data is found, use default
    if ($ip_data === null) {
        $ip_data = [
            'ip' => $ip,
            'firstseen' => $currtime,
    ...
    // Variable to track if the IP was found in the cluster
    $found = false;

    // Iterate through the existing IPs in the player cluster
    foreach ($player_cluster['IPs'] as &$ip_entry) {
        if (isset($ip_entry['indicator']) && $ip_entry['indicator'] === $ip_data['ip']) {
            // If the IP is already synced, don't update the lastseen field
            if (!$ip_entry['is_sync']) {
    ...
    // If the IP was not found, create a new entry
    if (!$found) {
        $new_ip_entry = array(
            ...
            'is_sync' => false, // New IP entry is not synced by default
        );

        // Add the new IP entry to the player cluster
...
function store_player_cluster($player_cluster)
{
    ...
    $data_json = json_encode([
        ...
        'VERSIONs'      =>$player_cluster['VERSIONs']
...
function get_player_cluster($uuid, &$player_cluster)
{
    ...
    if ($result) {
        ...
        $player_cluster = [
            ...
            'STEAMIDs'      => isset($data['STEAMIDs']) ? $data['STEAMIDs'] : array(),
            'IPs'           => isset($data['IPs']) ? $data['IPs'] : array(),
            'ACCOUNTs'      => isset($data['ACCOUNTs']) ? $data['ACCOUNTs'] : array(),
            'PAIRs'         => isset($data['PAIRs']) ? $data['PAIRs'] : array(),
            'OLDUKEY'       => isset($data['OLDUKEY']) ? $data['OLDUKEY'] : array(),
            'USERAGENTs'    => isset($data['USERAGENTs']) ? $data['USERAGENTs'] : array(),
            'PUNISHLOGs'    => isset($data['PUNISHLOGs']) ? $data['PUNISHLOGs'] : array(),
...
function get_ip_lang($ip)
{
    $ipdata = get_ip_data($ip);
    return isset($ipdata['location_country_code']) ? $ipdata['location_country_code'] : null;
}
...
function sync_indicators(&$player_cluster, &$chainstate, &$queriedIndicators, &$checked_uuids) {
    ...
    // Combine STEAMIDs and IPs into a single array and flag each entry
    $allIndicators = array_merge(
        array_map(function ($steamid) {
            $steamid['type'] = 'STEAMID'; // Flag as STEAMID
            return $steamid;
        }, $player_cluster['STEAMIDs']),
        array_map(function ($ip) {
            $ip['type'] = 'IP'; // Flag as IP
            return $ip;
        }, $player_cluster['IPs'])
    );

    // Filter out indicators that have already been queried and those marked as VPN/proxy/datacenter.
    // Apply age check only for IPs (entries flagged as 'IP').
    $Indicators = array_filter($allIndicators, function ($entry) use ($queriedIndicators) {
        // Skip if the indicator has already been queried
        if (in_array($entry['indicator'], $queriedIndicators)) {
            return false;
        }

        // Check for VPN, proxy, or datacenter for all entries
        if (check_vpn_or_proxy_idc($entry)) {
            return false;
        }

        // Apply age check only for IP entries (entries flagged as 'IP')
        if ($entry['type'] === 'IP' && isset($entry['lastseen'])) {
            $lastseen = DateTime::createFromFormat('Y-m-d H:i:s', $entry['lastseen']);
            $now = new DateTime();
            $interval = $now->diff($lastseen);
            if ($interval->days > MAX_IP_AGE_DAYS) {
                return false; // Skip if the IP is older than MAX_IP_AGE_DAYS
            }
        }

        return true; // Keep STEAMIDs and valid IPs for syncing
    });
    ...
    // Extract just the indicator values for the query
    $indicatorValues = array_column($Indicators, 'indicator');
    ...
    $conditions = [];
    foreach ($indicatorValues as $indicator) {
    ...
    $queriedIndicators = array_merge($queriedIndicators, $indicatorValues);
    ...
    foreach ($results as $result) {
        ...
        // Determine if data should sync based on matched indicator
        $should_sync = false;

        // 1. If the result is based on UUID from PAIRs, sync immediately
        if (isset($player_cluster['PAIRs']) && in_array($uuid, $player_cluster['PAIRs'])) {
            $should_sync = true;
        }

        // Check if STEAMID indicator matched
        if (!$should_sync) {
            foreach ($data['STEAMIDs'] as $steamid_entry) {
                if (in_array($steamid_entry['indicator'], $indicatorValues)) {
                    $should_sync = true; // Sync immediately if STEAMID matches
                    break;
                }
            }
        }
        ...
        if (!$should_sync) {
            foreach ($data['IPs'] as $ip_entry) {
                if (in_array($ip_entry['indicator'], $indicatorValues)) {
                    // If IP matched, check the MAX_IP_AGE_DAYS rule
                    $lastseen = DateTime::createFromFormat('Y-m-d H:i:s', $ip_entry['lastseen']);
                    $now = new DateTime();
                    $interval = $now->diff($lastseen);
                    if ($interval->days <= MAX_IP_AGE_DAYS) {
                        $should_sync = true;
        ...
        foreach ($data['IPs'] as &$ip_entry) {
            if (!$ip_entry['is_sync'] && !in_array($ip_entry['indicator'], array_column($player_cluster['IPs'], 'indicator'))) {
                // Check if the IP's lastseen is within the MAX_IP_AGE_DAYS range
                if (isset($ip_entry['lastseen'])) {
                    $lastseen = DateTime::createFromFormat('Y-m-d H:i:s', $ip_entry['lastseen']);
                    $now = new DateTime();
                    $interval = $now->diff($lastseen);
                    if ($interval->days > MAX_IP_AGE_DAYS) {
                        continue; // Skip IP if older than MAX_IP_AGE_DAYS
                    }
                }
...
{
    global $runtime, $bids, $vpn_or_proxy, $player_is_nonsteam, $query_ran, $indicators, $player_cluster, $Errors, $currtime, $additional, $localIp;
...
// Append other key information
        ...
        sendDiscordAlert(isset($player_cluster['UUID']) ? $player_cluster['UUID'] : $localIp, $runtime_time, $report_filename, $connection_status, $shutdown_reason);
    }
}
...
function sendDiscordAlert($uuid, $executionTime, $report_filename, $connection_status, $shutdown_reason)
{
    ...
    $reportBaseUrl = "https://herboyd2.hu/ban_api/reports.php?report=";
    ...
    $desc = (!empty($Errors) ? "There was an error during execution." : ($connection_status === CONNECTION_NORMAL ? 'Execution time exceeded 10 seconds.' : $shutdown_reason));

    // If there are errors, format them into a list as a code block
    $error_list = '';
    if (!empty($Errors)) {
        $error_list .= "<@425001439660605442> Errors:\n```ml\n";
...
function check_user_version($indicators, $g_version)
{
    $g_version_parts = explode(',', $g_version);

    if (count($g_version_parts) !== 3) {
        triggerCustomError(E_USER_WARNING, 'Invalid version format: ' . $g_version, __FILE__, __LINE__);
        return false;
    }
    
    $temp_Version = filter_var($g_version_parts[0], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $temp_Protocol = filter_var($g_version_parts[1], FILTER_SANITIZE_NUMBER_INT);
    $temp_Build = filter_var($g_version_parts[2], FILTER_SANITIZE_NUMBER_INT);

    if (strlen($temp_Version) < 2 || strlen($temp_Build) < 2 || strlen($temp_Protocol) < 2) {
        triggerCustomError(E_USER_WARNING, 'Invalid version parts length: ' . $g_version, __FILE__, __LINE__);
        return false;
    }

    $cl_versions = [
        'v_sVersion' => $temp_Version,
        'v_iBuild' => (int)$temp_Build,
        'v_iProtocol' => (int)$temp_Protocol,
        'v_sFullString' => $g_version
    ];

    global $pdoban;
    $stmt = $pdoban->prepare("
        SELECT * FROM version_restrict
    ");
    $stmt->execute();
    ...
    foreach ($results as $result) {
        ...
        foreach ($indicators as $indicator) {
            if ($indicator['type'] === IND_STEAMID && $indicator['value'] === $steamid) {
                if ($type === 'lower' && $cl_versions['v_iBuild'] > $version_build) {
                    $allowed_version = false;
                } elseif ($type === 'equal' && $cl_versions['v_iBuild'] !== $version_build) {
                    $allowed_version = false;
                } elseif ($type === 'higher' && $cl_versions['v_iBuild'] < $version_build) {
                    $allowed_version = false;
                }
            }
        }
...
try {
    ...
    $pdoreg = new PDO("mysql:host=$mysql_host;dbname=$mysql_dbreg", $mysql_user, $mysql_pass);
    $pdoreg->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    $pdoban = new PDO("mysql:host=$mysql_host;dbname=$mysql_dbban", $mysql_user, $mysql_pass);
...
if(!isset($player_cluster['VERSIONs']))
    $player_cluster['VERSIONs'] = array();

$allowed_version = check_user_version($indicators, $g_version);
...
if($vpn_or_proxy && $player_is_nonsteam && $g_steam != "STEAM_1:0:235393486" && $g_steam != "STEAM_1:0:1321169866")
{
...
running_time("(socket)LANG", 'sending');
$langstr = get_ip_lang($g_ipv4);
if($langstr != null)
{
    $passargsLANG = array(
        "i", $gid, 
    ...
    $skres = sck_CallPawnFunc("BanSystemv4.amxx", "SetLangauge", $passargsLANG);
    if($skres != 1 && $skres != 0)
        triggerCustomError(E_USER_WARNING,'(socket)LANG ret:'.$skres, __FILE__, __LINE__);
    running_time("(socket)LANG", 'sent');
}
else
{
    running_time("(socket)LANG", 'unset');
    triggerCustomError(E_USER_WARNING,'(get_ip_lang) ipdata is empty'.$skres, __FILE__, __LINE__);
}

if(!$allowed_version)
{
    running_time("(socket)version_kick", 'sending');
    $passargs = array("i", $gid, "i", $g_validator);
    $skres = sck_CallPawnFunc("BanSystemv4.amxx", "version_kick", $passargs);
    if($skres != 1 && $skres != 0)
        triggerCustomError(E_USER_WARNING,'(socket)version_kick ret:'.$skres, __FILE__, __LINE__);
    running_time("(socket)version_kick", 'sent');
    $player_cluster['PUNISHLOGs'][] = [
        'tpye' => "VERSION",
        'indicator' => $g_version,
        'time' => $currtime
...
{
    //missing useragent, in any known case it throws the cookie away
    $passargs = array("i", $gid, "i", $g_validator, "i", 3);
    $skres = sck_CallPawnFunc("BanSystemv4.amxx", "validationfailed", $passargs);
}
if(isset($_SERVER['HTTP_USER_AGENT']) && $_SERVER['HTTP_USER_AGENT'] == "Mozilla/4.0 (compatible; Synapse)")
{
    //known agent that throw cookie away
    $passargs = array("i", $gid, "i", $g_validator, "i", 4);
    $skres = sck_CallPawnFunc("BanSystemv4.amxx", "validationfailed", $passargs);
}

//******************************************************
// STAGE 5 - OPTIMALIZE DATA
//******************************************************
running_time("OPTIMALIZE 5","STORE DATA");
function OptimalizeCluster($player_cluster)
{
    global $currtime, $pdoban;
    if (!isset($player_cluster['INSTALLs'])) {
        $player_cluster['INSTALLs'] = [];
    }
    
    // Iterate over each UUID in PAIRs
    foreach ($player_cluster['PAIRs'] as $pair_uuid) {
        try {
            // Fetch the related data from MySQL
            $stmt = $pdoban->prepare("
                SELECT firstseen, lastseen, data 
                FROM amx_cookie_v2 
                WHERE UUID = :uuid
            ");
            $stmt->execute([':uuid' => $pair_uuid]);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
    
            if (!$result) {
                continue; // Skip if UUID is not found in the database
            }
    
            $data = json_decode($result['data'], true);
    
            // Find the earliest and latest timestamps
            $earliest = $result['firstseen'];
            $latest = $result['lastseen'];

            // Add to INSTALLs array
            $player_cluster['INSTALLs'][] = [
                'UUID' => $pair_uuid,
                'earliest' => $earliest,
                'latest' => $latest,
                'merged' => $currtime, // Current timestamp
            ];
    
            // Remove from PAIRs
            $player_cluster['PAIRs'] = array_values(array_diff($player_cluster['PAIRs'], [$pair_uuid]));
    
            // Delete the entry from MySQL
            //$stmt = $pdoban->prepare("DELETE FROM amx_cookie_pair WHERE UUID1 = :uuid OR UUID2 = :uuid");
            //$stmt->execute([':uuid' => $pair_uuid]);
        } catch (PDOException $e) {
            triggerCustomError(E_USER_WARNING, 'Error processing UUID ' . $pair_uuid . ': ' . $e->getMessage(), $e->getFile(), $e->getLine());
        }
    }
...
{
    $steamid_count = count($player_cluster['STEAMIDs']);
    $paris_count = count($player_cluster['PAIRs']);

    $hasSyncedSteamID = false;
    foreach ($player_cluster['STEAMIDs'] as $steamid) {
        if (isset($steamid['is_sync']) && $steamid['is_sync'] === true) {
            $hasSyncedSteamID = true;
            break;
        }
    }

    if(!$hasSyncedSteamID && $paris_count > 0)
    {
        // $additional .= "[PRE]player_cluser: " . json_encode($player_cluster, JSON_PRETTY_PRINT);
        // $additional .= "----------------------------------------------------------------------";
        // $additional .= "[POST]player_cluser: " . json_encode(OptimalizeCluster($player_cluster), JSON_PRETTY_PRINT);
        // triggerCustomError(E_USER_WARNING, 'REPORT CREATED FOR EXPERIMENTAL MERGING', __FILE__, __LINE__);
    }
}
...
// STAGE 6 - STORE DATA
//******************************************************
running_time("STAGE 6","STORE DATA");
